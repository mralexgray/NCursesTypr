...
@property (nonatomic) BOOL commandMode, searchMode;
@property (nonatomic) NSString* searchString;
@property (nonatomic) NSArray* copiedText;

@property (nonatomic) TextEditorView* textEditorView;
@property (nonatomic) TabMenuView* tabMenuView;
@property (nonatomic) NCLabel* modeLabel;
@end
...
- (void)viewDidLoad {
  ...
  for (NSString* openFile in NSProcessInfo.processInfo.arguments) {
    FileBuffer* buffer = [FileBuffer fileBufferFromFilePath:openFile];
    if (buffer)
      [self.tabMenuView addMenuItem:[NSFileManager.defaultManager.currentDirectoryPath stringByAppendingPathComponent:openFile.lastPathComponent] tag:buffer];
  }
}
...
- (void)didSelectFile:(NSString*)file
  ...
  if (openBuffer) {
    ...
    buffer ? [self.tabMenuView addMenuItem:file.lastPathComponent tag:buffer] : [self failedToOpenFile:file];
